...
目前已完成的 TPU evidence 包含 small public smoke run、Imagenette 320
validation-manifest inference timing tables，以及 single-device vs explicit
sharded TPU inference table families。這些仍不是 training、full controlled
...
- 後續 single-device vs explicit sharded TPU inference JSON artifacts 也已取回，
  範圍包含 `val256` 與 `val_full` 的 `single_v6e1` / `sharded_v6e8` table
  families。
