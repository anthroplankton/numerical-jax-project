...
cloud TPU public-example smoke-run comparison. The raw JSON benchmark outputs
live under ignored `runs/vit-inference/` and are treated as generated local
artifacts. Optional classifier-head fine-tuning smoke artifacts live under
ignored `runs/vit-finetune/`. Do not normally commit raw JSON outputs,
checkpoints, logs, predictions, or uncurated fine-tuning artifacts.
